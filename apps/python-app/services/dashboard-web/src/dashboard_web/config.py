...
def dashboard_settings() -> DashboardSettings:
    ...
    * ``PACKAGES_SERVICE_URL`` / ``ISSUES_SERVICE_URL`` / ``GRAPH_SERVICE_URL`` —
      upstream capability services (default to the in-cluster `Service` DNS names).
    ...
    * ``QUARANTINE_NAMESPACE`` — the namespace treated as "mirrored".
    ...
    return DashboardSettings(
        ...
        graph_service_url=os.environ.get(
            "GRAPH_SERVICE_URL", "http://graph-service"
        ).rstrip("/"),
